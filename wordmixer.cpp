...
WordMixer::WordMixer(const QString &fname)
    : fileName{fname},                                                                              // absolute path of the file which contains the mixed words
      rowNumber{-1},                                                                                // will be assigned a new (random) value when generating the number of the row which will be read from file by calling getRowNumber()
      totalRows{0},                                                                                 // will be updated by constructor when reading all rows from the file
      wordPieceSize{2},                                                                             // default size of the pieces in which the word is divided
      firstWord{},                                                                                  // initialized as empty string, will be assigned a value when the words are retrieved from file by calling retrieveWords()
      secondWord{},                                                                                 // same for the second word
      mixedWords{},                                                                                 // initially empty, will be filled in when the words are mixed by calling mixWords()
      areSynonyms{true},                                                                            // by default the words are synonyms (this is just for having the variable initialized)
      ...
      obtainedScore{0},                                                                             // all statistics (scores, number of pairs) initially set to 0 (this is reflected in the scores and number of pairs texts too)
      totalAvailableScore{0},
      guessedWordPairs{0},
      totalWordPairs{0},
      scoreIncrement{2},                                                                            // default level is medium, the number by which the scores are increased coresponds to this level
      highScoresMessage{"High-score: 0/0"},
      nrOfPairsMessage{"Word pairs: 0/0"}
{
    ...
    QFileInfo wordPairsFileStatus{fileName};                                                        // check if the file containing the word pairs exists, if not throw an exception that will end the application
...
{
    retrieveWords();                                                                                // read a new pair of words from the file
                                                                                                    // then retrieve the number of pieces each word consists of
    int firstWordSize{firstWord.size()}, secondWordSize{secondWord.size()};
    int firstWordNrOfPieces
    {
        firstWordSize / wordPieceSize + ((firstWordSize % wordPieceSize) == 0 ? 0 : 1)
    };
    int secondWordNrOfPieces
    {
        secondWordSize / wordPieceSize + ((secondWordSize % wordPieceSize) == 0 ? 0 : 1)
    };
    int totalNrOfPieces{firstWordNrOfPieces + secondWordNrOfPieces};
    mixedWords.resize(totalNrOfPieces);
    ...
    for (int pieceNumber{0}; pieceNumber<totalNrOfPieces; pieceNumber++)
    {
        wordPieceIndexes[pieceNumber] = pieceNumber;                                                // initial value of each element is equal to its index (it's the index of the coresponding element in mixedWords)
                                                                                                    ...
                                                                                                    */
    int firstWordLastPiecePos{wordPieceSize*(firstWordNrOfPieces-1)};                               // substring start position of the last piece of the first word
    int secondWordLastPiecePos{wordPieceSize*(secondWordNrOfPieces-1)};                             // substring start position of the last piece of the second word
    ...
    wordsBeginEndPositions[FIRST_BEGIN] =                                                           // insert the first piece of the first word and store the position index for further use
        insertWordPiece(firstWord, 0, wordPieceIndexes);
                                                                                                    // then continue inserting the other pieces of the first word until the last one is reached
    for (int wordPieceStartPos{wordPieceSize}; wordPieceStartPos<firstWordLastPiecePos;
                                      wordPieceStartPos+=wordPieceSize)
    {
        insertWordPiece(firstWord, wordPieceStartPos, wordPieceIndexes);
    }
    wordsBeginEndPositions[FIRST_END] =                                                             // insert the last piece of the first word and store the position index for further use
                    insertWordPiece(firstWord, firstWordLastPiecePos, wordPieceIndexes);
    wordsBeginEndPositions[SECOND_BEGIN] =                                                          // insert the first piece of the second word and store the position index for further use
                    insertWordPiece(secondWord, 0, wordPieceIndexes);
                                                                                                    // then continue inserting the pieces of the second word until the last one is reached
    for (int wordPieceStartPos{wordPieceSize}; wordPieceStartPos<secondWordLastPiecePos;
                                      wordPieceStartPos+=wordPieceSize)
    {
        insertWordPiece(secondWord, wordPieceStartPos, wordPieceIndexes);
    }
    wordsBeginEndPositions[SECOND_END] =                                                            // insert the last piece of the second word and store the position index for further use
                  insertWordPiece(secondWord, secondWordLastPiecePos, wordPieceIndexes);
...
bool WordMixer::checkWords(const QString &firstWord, const QString &secondWord)
{
    ...
    {
        createErrorMessage(MISSING_WORDS);
        return INCORRECT_USER_INPUT;
    }
                                                                                                   // second, check if both words were correctly entered
    if (((firstWord == this -> firstWord) && (secondWord == this -> secondWord)) ||                // case 1: words entered correctly and in order
        ((firstWord == this -> secondWord) && (secondWord == this -> firstWord)))                  // case 2: words entered correctly and in reverse order
    {
        createSuccessMessage();
        return CORRECT_USER_INPUT;
    }
    createErrorMessage(INCORRECT_WORDS);                                                           // any other case: error message, incorrect input
    return INCORRECT_USER_INPUT;
}
...
{                                                                                                  // shared variable statusMessage is used as results message (when the user pushes the Show results button)
    ...
    statusMessage += "\t" + firstWord + "\n";
    statusMessage += "\t" + secondWord + "\n";
    statusMessage += "\nThe words are: \n\n\t";
    if (areSynonyms)
    {
...
void WordMixer::updateStatistics(const bool partialUpdate)
{
    ...
    {
        obtainedScore += scoreIncrement;
        guessedWordPairs++;
    }
    totalAvailableScore += scoreIncrement;                                                         // total available number of points and total number of pairs presented to the user will be updated
    totalWordPairs++;                                                                              // anytime the user enters the words correctly (and pressses Submit) or he presses Show results button
    createHighScoresMessage();                                                                     // also update the texts to be written in the high-scores and number of pairs labels of the main game window
    createNrOfPairsMessage();
}
...
void WordMixer::resetStatistics()
{
    ...
    guessedWordPairs = 0;
    totalWordPairs = 0;
    createHighScoresMessage();                                                                     // after reset, texts are recalculated to reflect the new values in the scores and number of pairs labels
    createNrOfPairsMessage();
    statusMessage = "\nScores reset.\n\nLevel unchanged.\n\nNo user input so far.\n";              // message to be written in the results/errors box in main game window
                                                                                                   ...
                                                                                                   */
void WordMixer::getRowNumber()
{
    std::uniform_int_distribution<int> rowNumberDist{0,totalRows-1};                               // random number will be between 0 (first row) and the total number of rows minus 1 (last row)
    rowNumber = rowNumberDist(rowNumberEngine);
}
                                                                                                   ...
                                                                                                   */
void WordMixer::retrieveWords()
{
    getRowNumber();                                                                                // get the number of the file row from which the words will be read
    const int minWordSize{5};                                                                      // minimum allowed number of characters in a word
    QString rowNumberToString{};
    rowNumberToString.setNum(rowNumber);                                                           // convert the row number to a string for passing it when throwing exceptions
                                                                                                   // open the file; throw fatal exception (program ends) if file cannot be opened
    QFile wordPairsFile(fileName);
    if (!wordPairsFile.open(QIODevice::ReadOnly | QIODevice::Text))
    {
    ...
    firstWord = rowContent.left(separatorIndex);
    for (auto currentCharacter : firstWord)                                                        // check if first word includes only small letters; no other characters (numbers, spaces, etc) allowed
    {
        if (!(currentCharacter.isLower()))
        throw QString{"First word contains illegal characters!"} +
                      "\nRow number: " + rowNumberToString + "\n";
    }
    if (firstWord.size() < minWordSize)                                                            // check if first word has the minimum required number of characters
    {
        throw QString{"First word has less than the minimum required number of characters!"} +
                      "\nRow number: " + rowNumberToString + "\n";

    }
    secondWord = rowContent.mid(separatorIndex+1);
    for (auto currentCharacter : secondWord)                                                       // check if second word includes only small letters; no other characters (numbers, spaces, etc) allowed
    {
        if (!(currentCharacter.isLower()))
        throw QString{"Second word contains illegal characters!"} +
                      "\nRow number: " + rowNumberToString + "\n";
    }
    if (secondWord.size() < minWordSize)                                                           // check if second word has the minimum required number of characters
    {
        throw QString{"Second word has less than the minimum required number of characters!"} +
                      "\nRow number: " + rowNumberToString + "\n";
                                                                                                   ...
                                                                                                   */
void WordMixer::createErrorMessage(const int errorCode)
{
    switch(errorCode)
    {
    case MISSING_WORDS:
    ...
    default:
        ;                                                                                          // reserved for future use
                                                                                                   ...
                                                                                                   */
void WordMixer::createSuccessMessage()
...
void WordMixer::createHighScoresMessage()
{
    highScoresMessage = "High-score: " + QString::number(obtainedScore) + "/" +
                                         QString::number(totalAvailableScore);
}
                                                                                                   ...
                                                                                                   */
void WordMixer::createNrOfPairsMessage()
{
    nrOfPairsMessage = "Word pairs: " + QString::number(guessedWordPairs) + "/" +
                                        QString::number(totalWordPairs);
}
                                                                                                   ...
                                                                                                   */
int WordMixer::insertWordPiece(const QString &word, int firstCharPos,
                                        QVector<int> &wordPieceIndexes)
{
    ...
    int insertOnPositionIndex{indexDist(indexEngine)};
    ...
    mixedWords[insertOnPosition] = word.mid(firstCharPos,wordPieceSize);
